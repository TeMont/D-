...
void compiler::compLet(const node::StmtLet &stmtLet)
{
	...
	std::string varType = parser::letToType[stmtLet.letType];
	...
	if (varType == BOOL_TYPE)
	{
		...
		{
			if (!compExpr(*stmtLet.Expr, INT_TYPE) && !compExpr(*stmtLet.Expr, CHAR_TYPE) &&
			    !compExpr(*stmtLet.Expr, BOOL_TYPE))
			{
				std::cerr << "[Compile Error] ERR010 Expression Must Have Bool Type (Or Convertable To It)";
				exit(EXIT_FAILURE);
			}
	...
	else
	{
		if (stmtLet.Expr != nullptr)
		{
			if (!compExpr(*stmtLet.Expr, varType))
			{
				std::cerr << "[Compile Error] ERR006 Value Doesnt Matches Type";
				exit(EXIT_FAILURE);
			}
