...
std::unordered_map<Tokens, std::string>parser::letToType = {{Tokens::INT_LET, INT_TYPE},
		...
		else if (peek().value().type == Tokens::NOT)//FOR EXPRESSIONS LIKE !20, !x
		{
			...
			else
			{
				return {};
		         ...
		         peek().value().type == Tokens::IDENT && peek(1).has_value() &&
		         (peek(1).value().type == INC || peek(1).value().type == DEC))
		{
...
std::optional<node::Expr> parser::parseExpr(const std::string &expectedType, bool isRequired, uint8_t minPriority)
{
	...
	while (true)
	{
		if (!peek().has_value() || !op_to_prior(peek().value().type).has_value() ||
		    op_to_prior(peek().value().type) < minPriority)
		{
			break;
		}
...
std::optional<node::StmtLet> parser::parseLet()
{
	bool isConst = false;
	Tokens letType;
	if (peek().value().type == CONST)
	{
		isConst = true;
		consume();
		letType = consume().type;
	}
	else if (peek(1).has_value() && peek(1).value().type == CONST)
	{
		isConst = true;
		letType = consume().type;
		consume();
	...
	{
		consume();
		if (auto nodeExpr = parseExpr(letToType[letType]))
		{
...
std::optional<node::Stmt> parser::parseStmt(bool expectSemi)
{
	...
	{
		auto letStmt = parseLet().value();
		stmtNode = {letStmt};
		m_vars.insert({letStmt.ident.value.value(), letToType[letStmt.letType]});
	}
	else if (peek().value().type == Tokens::INC || peek().value().type == Tokens::DEC ||
