...
std::vector<std::vector<std::string>> typeArr = {{INT_TYPE, STR_TYPE, BOOL_TYPE, CHAR_TYPE},
...
std::vector<std::vector<Token>> litArr = {{{INT_LITERAL, "val"}},
...
TEST(ParserTest, parseValExprTest)
{
	for (int i = 0; i < typeArr.size(); ++i)
	{
		...
		{
			parser p({});
			ASSERT_FALSE(p.parseValExpr(typeArr[i][j])) << "ERROR Successfully parsed without literal";
			for (const auto &k: litArr[i])
			{
				p.pushToken(k);
			}
			...
			for (int k = 0; k < typeArr[i].size(); ++k)
			{
				for (auto &l: binOpArr)
				{
					...
					if (litArr[i][0].type == IDENT && litArr[j][0].type == IDENT || i == j && k == 0 ||
					    typeArr[i][k] == BOOL_TYPE && typeArr[i][0] != STR_TYPE && typeArr[j][0] != STR_TYPE)
					{
					...
					{
						if (litArr[i][0].type == IDENT && typeArr[i][k] == typeArr[j][0] ||
						    litArr[j][0].type == IDENT && typeArr[i][k] == typeArr[i][0])
						{
							auto parsedExpr = p.parseExpr(typeArr[i][k]);
